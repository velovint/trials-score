...
class MockCardScannerServiceTest {
    ...
    fun extractScores_returnsSuccess_withAllScores() = runTest {
        // Setup: Create a mock bitmap (content doesn't matter for mock)
        val mockBitmap = mock(Bitmap::class.java)

        // Action: Call extractScores
        val result = service.extractScores(mockBitmap)
    ...
    fun extractScores_returnsPredictableScores() = runTest {
        ...
        // Action
        val result = service.extractScores(mockBitmap) as ScanResult.Success
    ...
    @Test
    fun extractScores_ignoresimageBitmap() = runTest {
        // Setup: Call with null bitmap should still work (mock ignores it)
        ...
        val result1 = service.extractScores(mockBitmap1) as ScanResult.Success
        val result2 = service.extractScores(mockBitmap2) as ScanResult.Success
