...
object CardImagePreprocessor {
    ...
    // Tuned for camera-captured images where card fills most of frame
    private const val MIN_CARD_AREA_RATIO = 0.15  // Card must be at least 15% of image
    private const val CANNY_THRESHOLD1 = 50.0      // Edge detection threshold (higher = less sensitive)
    private const val CANNY_THRESHOLD2 = 150.0     // Edge detection threshold (higher = less sensitive)
    private const val GAUSSIAN_BLUR_SIZE = 5

    // Centering validation thresholds (relaxed for tight camera crops)
    // Allow cards to extend to any edge - camera crops are typically tight
    // The area threshold (>15%) and aspect ratio checks provide sufficient validation
    private const val MIN_CENTER_X_RATIO = 0.0  // Allow cards touching left edge
    private const val MIN_CENTER_Y_RATIO = 0.0  // Allow cards touching top edge
    ...
    fun preprocessImage(image: Mat): Mat {
        ...
        // Step 2: Auto-rotate to portrait if landscape (score cards are always portrait)
        ...
        // Step 3: Detect and crop card boundaries (if enabled)
        val cropped = if (ENABLE_CARD_DETECTION) {
            detectAndCropCard(oriented)
        } else {
            oriented
        }

        // Step 4: Resize to standard width for consistent processing
        val resized = Mat()
        ...
        // Release oriented if it's different from grayscale
        if (oriented !== grayscale) {
            oriented.release()
        }
        // Release cropped if it's different from oriented
        if (cropped !== oriented) {
            cropped.release()
        }

        Log.i("CardImagePreprocessor", "Preprocessed image: ${resized.width()}×${resized.height()}, channels=${resized.channels()}")
        return resized
    }
    ...
    private fun detectAndCropCard(image: Mat): Mat {
        ...
        try {
            // Save input grayscale image if debugging
            if (DEBUG_MODE && DEBUG_OUTPUT_DIR != null) {
                saveDebugImage(image, "input_grayscale")
            }
            ...
            // Reduced kernel size to avoid merging unrelated edges
            ...
            Log.i("CardImagePreprocessor", "  Card detection: imageArea=$imageArea, minArea=$minArea, contours=${contours.size}")
            ...
            for ((index, contour) in contours.withIndex()) {
                ...
                if (DEBUG_MODE) {
                    Log.i("CardImagePreprocessor", "    Contour $index: area=$area (${(area / imageArea * 100).toInt()}% of image)")
                }
                if (area > largestArea) {
                    largestArea = area
            ...
            // If we found a suitable contour, validate and crop to its bounding rectangle
            if (largestContour != null) {
                ...
                // Centering check: allow cards at edges (>= 0) - area and aspect ratio provide main validation
                ...
                val isPortraitish = aspectRatio > 1.5   // Score cards are tall

                if (DEBUG_MODE) {
                    Log.i("CardImagePreprocessor", "    Validation: rect=(${boundingRect.x}, ${boundingRect.y}, ${boundingRect.width}×${boundingRect.height})")
                    Log.i("CardImagePreprocessor", "    Required: x>${image.width() * MIN_CENTER_X_RATIO} (${image.width() * MIN_CENTER_X_RATIO}), y>${image.height() * MIN_CENTER_Y_RATIO} (${image.height() * MIN_CENTER_Y_RATIO})")
                    Log.i("CardImagePreprocessor", "    aspectRatio=${"%.2f".format(aspectRatio)}, areaRatio=${"%.2f".format(areaRatio)}")
                    Log.i("CardImagePreprocessor", "    isCentered=$isCentered, isReasonableSize=$isReasonableSize, isPortraitish=$isPortraitish")
                }

                val isValidDetection = isCentered && isReasonableSize && isPortraitish
                ...
                if (isValidDetection) {
                    ...
                    Log.i("CardImagePreprocessor", "  Card detected: ${paddedRect.width}×${paddedRect.height} at (${paddedRect.x}, ${paddedRect.y}) - ${(largestArea / imageArea * 100).toInt()}% of image")
                    return Mat(image, paddedRect).clone()
                } else {
                    Log.i("CardImagePreprocessor", "  Card detection rejected: aspectRatio=${"%.2f".format(aspectRatio)}, centered=$isCentered, size=${"%.1f".format(areaRatio * 100)}%, portrait=$isPortraitish - using full image")
                    return image
                }
            } else {
                Log.i("CardImagePreprocessor", "  No card boundary detected (no contour > ${(MIN_CARD_AREA_RATIO * 100).toInt()}% of image), using full image")
                return image
            }
    ...
    private fun saveDebugImage(mat: Mat, name: String) {
        DEBUG_OUTPUT_DIR?.let { dir ->
            ...
            Log.i("CardImagePreprocessor", "  Debug image saved: $path")
        }
    }
    ...
    private fun detectLines(edges: Mat, imageWidth: Int): List<HoughLine> {
        ...
        try {
            ...
            Log.i("CardImagePreprocessor", "  HoughLinesP detected ${lines.rows()} raw lines")
            ...
            Log.i("CardImagePreprocessor", "  ${detectedLines.size} horizontal lines after angle filtering")
    ...
    private fun findEvenlySpacedLines(lines: List<Double>, targetCount: Int): List<Double> {
        ...
        if (bestLines.isNotEmpty()) {
            Log.i("CardImagePreprocessor", "  Best grid uniformity: ${"%.3f".format(bestUniformity)} (lower is better)")
        }
        return bestLines
    }
    ...
    private fun findGridLines(lines: List<HoughLine>, imageHeight: Int): List<Double> {
        if (lines.isEmpty()) {
            Log.i("CardImagePreprocessor", "  No horizontal lines detected, falling back to equal-height division")
            return emptyList()
        }
        ...
        Log.i("CardImagePreprocessor", "  Detected ${clusteredLines.size} clustered horizontal lines")
        ...
        if (gridLines.size >= MIN_GRID_LINES) {
            Log.i("CardImagePreprocessor", "  Found grid with ${gridLines.size} lines (${gridLines.size - 1} rows)")
            return gridLines
        }

        Log.i("CardImagePreprocessor", "  Could not find 15 evenly-spaced lines, falling back to equal-height")
        return emptyList()
    }
    ...
    private fun extractRowsFromGridLines(image: Mat, gridLines: List<Double>): List<Mat> {
        ...
        Log.i("CardImagePreprocessor", "  Extracted ${rows.size} rows from detected grid lines")
    ...
    fun extractRowImages(image: Mat): List<Mat> {
        if (!USE_HOUGH_TRANSFORM) {
            Log.i("CardImagePreprocessor", "  Hough Transform disabled, using equal-height division")
        ...
        Log.i("CardImagePreprocessor", "  Falling back to equal-height row extraction")
        return extractRowsEqualHeight(image)
    }
}
